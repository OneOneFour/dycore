...
logical, private :: atm_netcdf_op = .true. ! When this is true, atmospheric variables don't need domain to operate
...
public :: set_atm_netcdf_op

private :: lookup_field_w, lookup_axis, unique_axes

character(len=128) :: version = '$Id: fms_io.F90,v 10.1 2004/01/22 00:00:06 arl Exp $'
character(len=128) :: tagname = '$Name:  $'

contains

subroutine set_atm_netcdf_op(state)
...
! open temporary file for writing data only, each field is written to a separate file
     ...
     else if (ASSOCIATED(Current_domain) .AND. .NOT. atm_netcdf_op) then
