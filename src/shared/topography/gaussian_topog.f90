...
!    </DATA>
   ...
   integer, parameter :: maxmts = 1

   real, dimension(maxmts) :: height = 4000.
   real, dimension(maxmts) ::  olon  = 180.
   real, dimension(maxmts) ::  olat  = -45.
   real, dimension(maxmts) ::  wlon  = 70.
   real, dimension(maxmts) ::  wlat  = 8.
   real, dimension(maxmts) ::  rlon  = 0.
   real, dimension(maxmts) ::  rlat  = 0.
!   real, dimension(maxmts) ::  lat0  = 25.
!   real, dimension(maxmts) ::  lat1  = 65.
...
character(len=128) :: version = '$Id: gaussian_topog.f90,v 10.1 2004/01/22 00:00:06 arl Exp $'
...
integer :: i, j
real    :: olon, olat, wlon, wlat, rlon, rlat, lat0, lat1
...
! defaults and convert degrees to radians (dtr)
  ...
  lat0 =  -25.  ;  lat0=lat0*dtr
  lat1 =  -65.  ;  lat1=lat1*dtr
  !print *, lat0, lat1 !correct

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! compute wavenumber 2 mountain
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    do j=1,size(lat(:))
      dy = lat(j)   ! dist from y origin
    !  yy = max(0., dy-rlat)/wlat
    !  if ((dy < lat1) .and. (dy > lat0)) then 
        do i=1,size(lon(:))
          dx = abs(lon(i)) ! dist from x origin
          dx = min(dx, abs(dx-tpi))  ! To ensure that: -pi <= dx <= pi
    !      xx = max(0., dx-rlon)/wlon
          if ((dy .gt. lat1) .and. (dy .lt. lat0)) then
          zsurf(i,j) = height*(sin((dy-lat0)/(lat1-lat0)*pi))**2*cos(2*dx)
          elseif ((dy .lt. lat1) .or. (dy .gt. lat0)) then
          zsurf(i,j) = 0.
          endif
    !      print *, zsurf(i,j)
        enddo
