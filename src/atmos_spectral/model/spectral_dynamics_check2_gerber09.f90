...
public :: get_pk_bk, complete_robert_filter, complete_update_of_future, complete_init_of_tracers
public :: get_axis_id, spectral_diagnostics

!===============================================================================================

character(len=128), parameter :: version = '$Id: spectral_dynamics.f90,v 10.1 2004/01/22 00:00:05 arl Exp $'
...
real, allocatable, dimension(:,:,:) :: vorg, divg, wg    ! no time levels needed

integer, allocatable, dimension(:) :: tracer_vert_advect_scheme

real    :: virtual_factor
integer :: pe, npes, num_tracers, nhum, t_vert_advect_scheme, uv_vert_advect_scheme
...
subroutine spectral_dynamics_init(Time, Time_step_in, previous, current, ug, vg, tg, psg, wg_full, tracer_attributes, &
                                  grid_tracers, z_full, z_half, p_full, p_half, dry_model_out, nhum_out, ocean_mask)

type(time_type), intent(in) :: Time, Time_step_in
integer, intent(out) :: previous, current
real,    pointer, dimension(:,:,:,:) :: ug, vg, tg
real,    pointer, dimension(:,:,:) :: psg, wg_full
type(tracer_type), intent(inout), dimension(:) :: tracer_attributes
real,    pointer, dimension(:,:,:,:,:) :: grid_tracers
real, pointer, dimension(:,:,:) :: z_full, z_half, p_full, p_half
...
call allocate_fields(psg, ug, vg, tg, wg_full, z_full, z_half, p_full, p_half, grid_tracers)
...
call read_restart_or_do_coldstart(tracer_attributes, previous, current, ug, vg, tg, psg, wg_full, grid_tracers, ocean_mask)
...
!===============================================================================================
subroutine read_restart_or_do_coldstart(tracer_attributes, previous, current, ug, vg, tg, psg, &
                                        wg_full, grid_tracers, ocean_mask)
...
type(tracer_type), intent(inout), dimension(:) :: tracer_attributes
integer, intent(out) :: previous, current
real,    intent(out), dimension(is:ie, js:je, num_levels, num_time_levels) :: ug, vg, tg
real,    intent(out), dimension(is:ie, js:je,             num_time_levels) :: psg
real,    intent(out), dimension(is:ie, js:je, num_levels                 ) :: wg_full
real,    intent(out), dimension(is:ie, js:je, num_levels, num_time_levels, num_tracers) :: grid_tracers
logical, optional, intent(in), dimension(:,:) :: ocean_mask
...
! call read_data(trim(file), 'current',  current)            ! No interface of read_data exists to read integer scalars
  call read_data(trim(file), 'time_pointers', time_pointers) ! clug to getaround no interface to read integer scalars
  previous = int(time_pointers(1))                           ! clug to getaround no interface to read integer scalars
  current  = int(time_pointers(2))                           ! clug to getaround no interface to read integer scalars
  call read_data(trim(file), 'pk', pk)
  call read_data(trim(file), 'bk', bk)
  ...
  unit = open_restart_file('INPUT/spectral_dynamics.res','read')
  read(unit) previous, current
  read(unit) pk,bk
  call set_domain(spectral_domain)
  call read_data(unit,vors)
  call read_data(unit,divs)
  call read_data(unit,ts)
  call read_data(unit,ln_ps)
  call set_domain(grid_domain)
  call read_data(unit,ug)
  call read_data(unit,vg)
  call read_data(unit,tg)
  call read_data(unit,psg)
  call read_data(unit,vorg)
  call read_data(unit,divg)
  call read_data(unit,surf_geopotential)
  call read_data(unit,wg_full)
...
do ntr = 1,num_tracers
  ...
  if(file_exist('INPUT/tracer_'//trim(tr_name)//'.res.nc')) then
    file = 'INPUT/tracer_'//trim(tr_name)//'.res.nc'
    call field_size(trim(file), trim(tr_name), siz)
    if(lon_max /= siz(1) .or. lat_max /= siz(2) .or. num_levels /= siz(3)) then
      write(ch1,'(i4)') siz(1)
      write(ch2,'(i4)') siz(2)
      write(ch3,'(i4)') siz(3)
      ...
      ' Restart data: lon_max='//ch1//', lat_max='//ch2//', num_levels='//ch3// &
         '  Namelist: lon_max='//ch4//', lat_max='//ch5//', num_levels='//ch6, FATAL)
    endif
    if(uppercase(trim(tracer_attributes(ntr)%numerical_representation)) == 'SPECTRAL') then
      call field_size(trim(file), trim(tr_name)//'_real', siz)
      if(num_fourier /= siz(1)-1 .or. num_spherical /= siz(2)-1) then
        write(ch1,'(i4)') siz(1)-1
        write(ch2,'(i4)') siz(2)-1
        write(ch3,'(i4)') num_fourier
        write(ch4,'(i4)') num_spherical
        call error_mesg('spectral_dynamics_init','Resolution of restart data does not match resolution specified on namelist.'// &
        ' Restart data: num_fourier='//ch1//', num_spherical='//ch2// &
        '  Namelist: num_fourier='//ch3//', num_spherical='//ch4, FATAL)
      endif
    endif
    do nt=1,num_time_levels
    ...
    enddo
    tracer_attributes(ntr)%restarted = .true.
  else if(file_exist('INPUT/tracer_'//trim(tr_name)//'.res')) then
    unit = open_restart_file('INPUT/tracer_'//trim(tr_name)//'.res', 'read')
    call set_domain(grid_domain)
    call read_data(unit,grid_tracers(:,:,:,:,ntr))
    if(uppercase(trim(tracer_attributes(ntr)%numerical_representation)) == 'SPECTRAL') then
      call set_domain(spectral_domain)
      call read_data(unit,spec_tracers(:,:,:,:,ntr))
    endif
    tracer_attributes(ntr)%restarted = .true.
  else
    if(trim(tracer_attributes(ntr)%name) == 'sphum') then
      ...
      grid_tracers(:,:,:,:,ntr) = 0.
      call tracer_requires_init(MODEL_ATMOS, tracer_attributes(ntr)%name)
    endif
    tracer_attributes(ntr)%restarted = .false.
  endif
enddo
...
subroutine complete_init_of_tracers(tracer_attributes, previous, current, grid_tracers)
...
if(.not.module_is_initialized) then
  call error_mesg('complete_init_of_tracers','dynamics has not been initialized ', FATAL) 
endif

do ntr=1,num_tracers
! The check below works only if the initialization flag (needs_init) is flipped from .true. to .false.
! in the physics routine that does the initialization. At the time I write this, there is no public
! interface of tracer_manager that allows one to do this.

! if(query_tracer_init(MODEL_ATMOS, tracer_attributes(ntr)%name)) then
!   call error_mesg('complete_init_of_tracers','tracer "'//trim(tracer_attributes(ntr)%name)//'" has not been initialized', FATAL)
! endif
  if(.not.tracer_attributes(ntr)%restarted) then
    grid_tracers(:,:,:,previous,ntr) = grid_tracers(:,:,:,current,ntr)
    if(uppercase(trim(tracer_attributes(ntr)%numerical_representation)) == 'SPECTRAL') then
...
enddo
return
end subroutine complete_init_of_tracers

!===============================================================================================

subroutine allocate_fields(psg, ug, vg, tg, wg_full, z_full, z_half, p_full, p_half, grid_tracers)
real, pointer, dimension(:,:,:) :: psg, wg_full
real, pointer, dimension(:,:,:,:) :: ug, vg, tg
real, pointer, dimension(:,:,:) :: z_full, z_half, p_full, p_half
real, pointer, dimension(:,:,:,:,:) :: grid_tracers
...
psg=0.; ug=0.; vg=0.; tg=0.; wg_full=0.; z_full=0.; z_full=0.; z_half=0.; p_full=0.; p_half=0.
ln_ps=cmplx(0.,0.); vors=cmplx(0.,0.); divs=cmplx(0.,0.); ts=cmplx(0.,0.); spec_tracers=cmplx(0.,0.)
pk=0.; bk=0.; vorg=0.; divg=0.; wg=0.; surf_geopotential=0.; grid_tracers=0.
...
subroutine spectral_dynamics(Time_prev, Time, Time_next, psg, ug, vg, tg, tracer_attributes, grid_tracers, &
                             previous, current, future, dt_psg, dt_ug, dt_vg, dt_tg, dt_tracers, wg_full, p_full, &
                             p_half, z_full, timespinup)

type(time_type),  intent(in) :: Time_prev, Time, Time_next
type(tracer_type),intent(inout), dimension(:) :: tracer_attributes
real,    intent(inout), dimension(is:ie, js:je, num_levels, num_time_levels, num_tracers) :: grid_tracers

integer, intent(in) :: previous, current, future

real, intent(inout), dimension(is:ie, js:je,             num_time_levels) :: psg
real, intent(inout), dimension(is:ie, js:je, num_levels, num_time_levels) :: ug, vg, tg
...
real   , dimension(is:ie, js:je, num_levels+1) :: phig_half, ln_p_half
real, dimension(lat_max  ) :: lat


integer :: j, k, n, i, seconds, days, time_level
real    :: delta_t
real    :: timespinup, totalspinup, dummy, dummy2, dummy3, dummy4, dummy5, dummy6
real, dimension(40, 48) :: dh
...
call get_time(Time_next-Time_prev, seconds, days)
delta_t = 86400*days + seconds
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!! Added new code here !!!!!!!!!!!!!!!!!
totalspinup=20.*86400

!PRINT *, timespinup

call initialize_corrections(ug, vg, tg, grid_tracers, &
                            psg, dt_ug, dt_vg, dt_tg, dt_tracers, delta_t, previous, current)
...
call four_in_one (divg, ug(:,:,:,current), vg(:,:,:,current), virtual_t, &
           psg(:,:,current), ln_p_half, ln_p_full, p_full, dx_psg, dy_psg, dt_psg, wg, wg_full, dt_tg, dt_ug, dt_vg)
...
dt_tg = dt_tg + dt_grid_tmp

call horizontal_advection(ts(:,:,:,current), ug(:,:,:,current), vg(:,:,:,current), dt_tg)
!call trans_grid_to_spherical(dt_tg, dt_ts) ! moved to AFTER dt_tg+forcing

!do k=1,num_levels
!  do j = js,je
!    dt_ug(:,j,k) = dt_ug(:,j,k) + (vorg(:,j,k) + coriolis(j))*vg(:,j,k,current)
!    dt_vg(:,j,k) = dt_vg(:,j,k) - (vorg(:,j,k) + coriolis(j))*ug(:,j,k,current)
!  enddo
!enddo
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!! add momentum forcing here !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! do k=1,num_levels
!    do j = js,je
!     dt_ug(:,j,k) = dt_ug(:,j,k) - 1./86400*exp(-1./9*(j-8)**2)*timespinup/totalspinup &
!       *exp(-1./64*((log(bk(k))-log(50./1000))*20)**2) +  1./86400*exp(-1./9*(j-41)**2) &
!        *timespinup/totalspinup*exp(-1./64*((log(bk(k))-log(50./1000))*20)**2)
!   enddo
! enddo
...
! do k=1,num_levels
!   do j = js,je
!     dt_ug(:,j,k) =dt_ug(:,j,k)-1./86400*exp(-1./9*(j-8)**2)*exp(-1./64*((log(bk(k))-log(50./1000))*20)**2) + &
!        1./86400*exp(-1./9*(j-41)**2)*exp(-1./64*((log(bk(k))-log(50./1000))*20)**2)
!   enddo
!enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!                  ADD DIABATIC HEATING MAX - 2 K /DAY at peak lat               !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!do k=1,num_levels
!  do j=js,je
!    IF (lat(j)<10 .AND. lat(j)>-10) THEN
!
!       dt_tg(:,j,k)=dt_tg(:,j,k)+5.0/86400*exp(-(p_full(10,j,k)/100.-550.)**2/2/200**2)*(cos((j-24.5)/2.7))**2
!
!    ENDIF
!  enddo
!enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! matlab version
!for k=1:kk;for j=1:jj;Q(k,j)=exp(-(s(k)*1000-525)^2/2/200^2)*(cos(pi/2*(lat2(j)-10)/20))^2;end;end
!for k=1:kk;for j=1:jj;Q2(k,j)=2*exp(-(s(k)*1000-525)^2/2/200^2)*(cos((j-27)/4))^2;end;end;
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!




call vor_div_from_uv_grid(dt_ug, dt_vg, dt_vors, dt_divs, triang = triang_trunc)
...
call update_tracers(tracer_attributes, grid_tracers, dt_tracers, ug, vg, wg, p_half, previous, current, future, delta_t)

call compute_corrections(ug, vg, delta_t, current, future, tg, psg, tracer_attributes, grid_tracers)
...
subroutine update_tracers(tracer_attributes, grid_tracers, dt_tr, ug, vg, wg, p_half, previous, current, future, delta_t)

type(tracer_type), intent(inout), dimension(:) :: tracer_attributes
real   , intent(inout), dimension(:,:,:,:,:) :: grid_tracers
real   , intent(inout), dimension(:,:,:,:) :: dt_tr
real   , intent(in   ), dimension(:,:,:,:) :: ug, vg
...
subroutine compute_corrections(ug, vg, delta_t, current, future, tg, psg, tracer_attributes, grid_tracers)

real,              intent(in   ), dimension(:,:,:,:) :: ug, vg
real,              intent(in   )                     :: delta_t
integer,           intent(in   )                     :: current, future
real,              intent(inout), dimension(:,:,:,:) :: tg
...
subroutine initialize_corrections(ug, vg, tg, grid_tracers, psg, dt_ug, dt_vg, dt_tg, dt_tracers, delta_t, previous, current)
...
!================================================================================
subroutine complete_update_of_future(current, future, psg, ug, vg, tg, tracer_attributes, grid_tracers)
integer, intent(in) :: current, future
real,              intent(inout), dimension(:,:      ) :: psg
...
call vor_div_from_uv_grid(ug, vg, vors(:,:,:,future), divs(:,:,:,future), triang=triang_trunc)
call trans_spherical_to_grid(vors(:,:,:,future), vorg)
call trans_spherical_to_grid(divs(:,:,:,future), divg)
ln_psg = alog(psg)
call trans_grid_to_spherical(ln_psg, ln_ps(:,:,future))
call trans_grid_to_spherical(tg, ts(:,:,:,future))
do ntr=1,num_tracers
  if(uppercase(trim(tracer_attributes(ntr)%numerical_representation)) == 'SPECTRAL') then
    call trans_grid_to_spherical(grid_tracers(:,:,:,future,ntr), spec_tracers(:,:,:,future,ntr))
  endif
enddo
...
!================================================================================
subroutine complete_robert_filter(current, future, tracer_attributes, grid_tracers)
integer, intent(in) :: current, future
type(tracer_type), intent(inout), dimension(:) :: tracer_attributes
real,              intent(inout), dimension(:,:,:,:,:) :: grid_tracers
integer :: ntr

call leapfrog_2level_B(ln_ps, current, future, robert_coeff)
call leapfrog_2level_B(vors,  current, future, robert_coeff)
call leapfrog_2level_B(divs,  current, future, robert_coeff)
call leapfrog_2level_B(ts,    current, future, robert_coeff)
do ntr = 1, num_tracers
  if(uppercase(trim(tracer_attributes(ntr)%numerical_representation)) == 'SPECTRAL') then
    call leapfrog_2level_B(spec_tracers(:,:,:,:,ntr), current, future, robert_coeff)
  else
    call leapfrog_2level_B(grid_tracers(:,:,:,:,ntr), current, future, robert_coeff)
  endif
enddo
...
subroutine spectral_dynamics_end(previous, current, ug, vg, tg, psg, wg_full, tracer_attributes, &
...
call write_data(trim(file), 'time_pointers', (/real(previous),real(current)/)) ! clug to getaround no interface to write a scalar
call write_data(trim(file), 'pk', pk)
call write_data(trim(file), 'bk', bk)
do nt=1,num_time_levels
...
do ntr = 1,num_tracers
  tr_name = trim(tracer_attributes(ntr)%name)
  file='RESTART/tracer_'//trim(tr_name)//'.res.nc'
  do nt=1,num_time_levels
...
deallocate(ug, vg, tg, psg, wg_full)
deallocate(z_full, z_half, p_full, p_half)
...
!===================================================================================
subroutine spectral_diagnostics(Time, psg, ug, vg, tg, wg_full, grid_tracers)

type(time_type), intent(in) :: Time
real, intent(in), dimension(is:ie, js:je)                          :: psg
real, intent(in), dimension(is:ie, js:je, num_levels)              :: ug, vg, tg, wg_full
real, intent(in), dimension(is:ie, js:je, num_levels, num_tracers) :: grid_tracers
...
if(id_ps  > 0)    used = send_data(id_ps,   psg, Time)
if(id_u   > 0)    used = send_data(id_u,     ug, Time)
if(id_v   > 0)    used = send_data(id_v,     vg, Time)
...
if(id_zfull > 0 .or. id_zhalf > 0) then
  call compute_pressures_and_heights(tg, psg, z_full, z_half, p_full, p_half)
...
if(size(grid_tracers,4) /= num_tracers) then
  write(err_msg_1,'(i8)') size(grid_tracers,4)
  write(err_msg_2,'(i8)') num_tracers
  call error_mesg('spectral_diagnostics','size(tracers)='//err_msg_1//' Should be='//err_msg_2, FATAL)
endif
do ntr=1,num_tracers
  if(id_tr(ntr) > 0) used = send_data(id_tr(ntr), grid_tracers(:,:,:,ntr), Time)
enddo
...
if(id_slp > 0) then
  do j=js,je
    do i=is,ie
      do k=1,num_levels
        if(p_full(i,j,k)/psg(i,j) > .8) goto 20
      enddo
      call error_mesg('spectral_diagnostics','No sigma values .gt. 0.8  Cannot compute slp',FATAL)
20    continue
      t_low(i,j) = tg(i,j,k)*(p_full(i,j,k)/psg(i,j))**(-expf)
      slp(i,j) = psg(i,j)*((t_low(i,j) + gamma*surf_geopotential(i,j)/grav)/t_low(i,j))**expf_inverse
    enddo
  enddo
...
if(interval_alarm(Time, Time_step, Alarm_time, Alarm_interval)) then
  call global_integrals(Time, psg, ug, vg, tg, wg_full, grid_tracers)
endif

return
end subroutine spectral_diagnostics
!===================================================================================
subroutine global_integrals(Time, psg, ug, vg, tg, wg_full, grid_tracers)
type(time_type), intent(in) :: Time
real, intent(in), dimension(is:ie, js:je)                          :: psg
real, intent(in), dimension(is:ie, js:je, num_levels)              :: ug, vg, tg, wg_full
real, intent(in), dimension(is:ie, js:je, num_levels, num_tracers) :: grid_tracers
