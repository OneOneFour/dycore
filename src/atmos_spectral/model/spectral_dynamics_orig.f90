...
character(len=128), parameter :: version = '$Id: spectral_dynamics.f90,v 10.0 2003/10/27 23:31:04 arl Exp $'
...
allocate(dpk(size(pk)-1))
allocate(dbk(size(bk)-1))
...
subroutine spectral_dynamics(Time_prev, Time, Time_next, psg, ug, vg, tg, tracer_attributes, grid_tracers, &
                             previous, current, future, dt_psg, dt_ug, dt_vg, dt_tg, dt_tracers, wg_full, p_full, p_half, z_full)
...
integer :: j, k, n, seconds, days, time_level
real    :: delta_t
...
character(len=32) :: chtmp='size(pk)=1000  size(bk)=1000    '

if(.not.module_is_initialized) then
  call error_mesg('get_pk_bk', 'spectral_dynamics_init has not been called.', FATAL)
endif
if(size(pk_out) /= size(bk_out)) then
  write(chtmp(10:13),'(i4)') size(pk_out)
  write(chtmp(25:28),'(i4)') size(bk_out)
