...
'$Id: transforms.f90,v 10.0 2003/10/27 23:31:04 arl Exp $'
...
real, intent(inout), dimension (:,:) :: grid
real, intent(in), optional, dimension (0:num_fourier,0:num_spherical) :: filter
...
!-------------------------------------------------------------------------
  ...
  if (.not. global_tmp) then
     if(size(lon_boundaries) /= ie-is+2) then
        write(chtmp1,'(i3)') size(lon_boundaries)
     ...
     if(size(lat_boundaries) /= je-js+2) then
        write(chtmp1,'(i3)') size(lat_boundaries)
     ...
     if(size(lon_boundaries) /= num_lon+1) then
        write(chtmp1,'(i3)') size(lon_boundaries)
     ...
     if(size(lat_boundaries) /= lat_max+1) then
        write(chtmp1,'(i3)') size(lat_boundaries)
