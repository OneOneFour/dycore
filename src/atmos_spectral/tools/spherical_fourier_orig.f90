...
character(len=128), parameter :: version = '$Id: spherical_fourier.f90,v 10.0 2003/10/27 23:31:04 arl Exp $'
...
if(size(sin_lat_out).eq.lat_max) then
    sin_lat_out = sin_lat
...
if(size(cos_lat_out).eq.lat_max) then
    cos_lat_out = cos_lat
else                            !assume grid compute domain
...
if(size(cosm_lat_out).eq.lat_max) then
...
if(size(cosm2_lat_out).eq.lat_max) then
...
if(size(deg_lat_out).eq.lat_max) then
    deg_lat_out = deg_lat
else                            !assume grid compute domain
...
if(size(wts_lat_out).eq.lat_max) then
    wts_lat_out = wts_lat
else                            !assume grid compute domain
