...
!  ---- average mass weights for mass fluxes ----
   ...
   call change_grid ( Hgrid, TEMP_GRID, WIND_GRID, dpde, adpdxy )
...
!---- sum input/output mass fluxes -------
      ...
      do j = js-1, je+1
         udy(:,j)=Hgrid%Vel%dy   *u(:,j,k)*adpdxy(:,j,k)   *Masks%Vel%mask(:,j,k)
         vdx(:,j)=Hgrid%Vel%dx(j)*v(:,j,k)*adpdxy(:,j,k)   *Masks%Vel%mask(:,j,k)
      enddo
...
! second-order scheme (only the gradient)
  do k = ks, ke
     ...
     do i = is, ie
        ...
        v_dt(i,j,k) = v_dt(i,j,k) + (dns(i,j)+dns(i+1,j)) * dcoeff * Hgrid%Vel%dy
     enddo
     enddo
  enddo
