...
!---- version number ----
   ...
   character(len=128) :: version='$Id: bgrid_core.f90,v 10.0 2003/10/27 23:31:04 arl Exp $'
...
!    ---- polar filtering -----
     ...
     if (Dynam%fopt >= 1) then
       call polar_filter (Dynam%Pfilt, div, omgalf, TGRID, Dynam%Masks%Tmp%mask)
     endif
...
!------- compute sfc pres tendency, vert. alpha-omega, & vert vel.------
    ...
    call vert_adjust ( Dynam%Vgrid, Dynam%res, div, wta, wtb,     &
