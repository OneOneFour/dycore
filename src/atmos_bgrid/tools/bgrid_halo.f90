...
!  ----- check/set optional flag arguments ----
   ...
   call set_domain_flags ( Hgrid%decompx, halos, flags )
...
!  ------ need to determine and check grid -------
   ...
   select case (field)
     ...
     case (UWND:VWND)
        ...
        ! update non-polar boundaries
        if (.not.do_pole_only) call mpp_update_domains (data, Hgrid%Vel%Domain, domain_flags)
     case default
...
!#######################################################################
 ...
 subroutine set_domain_flags ( decompx, halos, flags )
 logical, intent(in)           :: decompx
 integer, intent(in), optional :: halos, flags
