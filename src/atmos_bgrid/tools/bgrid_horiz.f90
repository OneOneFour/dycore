...
!-------- internal parameters --------
   ...
   real :: lon_beg = 0.0  ! defines the western edge of the
                          ! temperature grid box with global index, i=1

!----- untested/unsupported options -----
!         use at you own risk 

   real :: tph0d = 0.  ! grid/globe transformation
   real :: tlm0d = 0.  ! (set tph0d=tlm0d=0 for no transformation)

   logical :: channel = .false. ! if TRUE, creates horizontal constants
                                !    for a very crude f-plane channel

!-----------------------------------------------------------------------
 character(len=128) :: version = '$Id: bgrid_horiz.f90,v 10.0 2003/10/27 23:31:04 arl Exp $'
...
real    :: hpi, dtr
integer :: i, j, ilb, iub, jlb, jub, npes, pe
...
!-----------------------------------------------------------------------
   ...
   Hgrid % channel = channel  ! unsupported channel model option
...
!------------- trigometric constants -----------------------------------
      ...
      if (Hgrid % channel) then
!     --- channel model ---
          Hgrid % sinphv = sin(45./dtr) ! arbitrary latitude chosen
          Hgrid % tanphv = 0.0
      else
!     --- sphere ---
