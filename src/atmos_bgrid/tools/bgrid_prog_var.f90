...
logical :: do_log = .true.
character(len=128) :: version='$Id: bgrid_prog_var.f90,v 10.0 2003/10/27 23:31:04 arl Exp $'
...
! read single tracer restart file
  ...
  if (file_exist(trim(filename_in))) then
      do n = 1, Var%ntrace
         ...
         call field_size (trim(filename_in), tr_name, siz)
         if (siz(1) == Hgrid%nlon .and. siz(2) == Hgrid%nlat .and. siz(3) == size(eta)-1) then
            call read_data (trim(filename_in), tr_name, Var%r(:,:,:,n), &
                            Hgrid%Tmp%Domain)
         else if (siz(1) == -1 .and. siz(2) == -1 .and. siz(3) == -1) then
...
!-----------------------------------------------------------------------
! output format
  oform = 'NATIVE'; if (present(format)) oform = format
